...
def show_python_code_and_response(response):
    """Process and display the response in a more readable format."""
    if not response:
        return "No response received"
        
    result = ""
    for item in response:
        if not hasattr(item, 'content') or not hasattr(item.content, 'parts'):
            continue
            
        for part in item.content.parts:
            if hasattr(part, 'text') and part.text:
                result += str(part.text) + "\n\n"
            elif (hasattr(part, 'function_response') and 
                ...
                if part.function_response.response is not None:
                    if isinstance(part.function_response.response, dict):
                        if 'result' in part.function_response.response:
                            result += str(part.function_response.response['result']) + "\n\n"
                    else:
                        result += str(part.function_response.response) + "\n\n"
    
    return result.strip() or "No content available in response"
...
async def main():
    # Create a runner for the enhanced currency agent
    ...
    response = await enhanced_runner.run_debug(
        "Convert 500 USD to EUR using a Platinum Credit Card"
    )
    show_python_code_and_result(response)
